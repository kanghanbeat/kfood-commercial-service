...
**메인 개편 1주차 완료 + UI 다국어(4개 국어) 추가: 내비 3카테고리 전환·음식 하위 탭, 게시판 토글 3종 확대, 음식×지역 조합 라우트, 메인 4섹션 뼈대, 그리고 헤더 지구본 언어 선택기(EN/KO/JA/ZH — UI 문구만, 쿠키 기반, URL 무변경).** 다음 작업: 메인 개편 2주차 — SVG 지도 컴포넌트 + 색칠 로직 + 시·도↔지역 매핑(A안).
...
1. **방향 결정 — 기준 문서 전면 갱신** (`docs/06-team/기획정렬-한빛대조.md` — 2026-07-09 저장소로 이동, 이전엔 솔 로컬):
   ...
   - 근거 조사: `K푸드_플랫폼/docs/벤치마크-레퍼런스.md` (일본 byFood·TasteAtlas·TABERUKOTO 등 15곳 — 같은 모델의 한국 대상 서비스 부재 확인)
...
3. **메인 개편 준비물 확보** — ① 한국 시·도 SVG 지도 생성: `K푸드_플랫폼/데이터수집/한국지도/korea-provinces.svg` (17개 시·도, 각 조각 영문 id, 109KB, 통계청 출처 상업 사용 가능) ② 주차별 작업 계획: `K푸드_플랫폼/docs/메인개편-작업계획.md`. ⚠️ 설계 결정 하나 남음: 지도(시·도 17개) ↔ regions(동네 단위 23개) 연결 — **A안** 코드 매핑표(스키마 무변경) vs **B안** regions에 province 필드 추가(스키마 변경 = 한빛 승인 필요). 착수는 A안으로 시작.
...
**향후 결정 사항 (시점만 정해둠)**: 다국어(i18n) 버튼 — 실콘텐츠 채운 후(1번 이후) 착수, 한국어·영어·일본어·중국어 4개 추천. 상세 근거는 `docs/06-team/기획정렬-한빛대조.md` 참고.
...
- docs/06-team/기획정렬-한빛대조.md  (방향 기준)
- docs/06-team/메인개편-작업계획.md  (작업 순서)
