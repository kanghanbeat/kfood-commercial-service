...
export default async function JourneyRecapPage() {
  const session = await requirePublicSession("/mypage/journey");
  await ensurePublicProfile(session);

  const [foods, regions, triedAt] = await Promise.all([
    getPublishedFoods(),
    getPublishedRegions(),
    ...
    <main className="page-shell">
      <header className="detail-header">
        ...
        <p className="detail-intro">
          A snapshot of the K-food collection you have tried so far. Photos and
          a shareable link are coming in a later update.
        </p>
      </header>
